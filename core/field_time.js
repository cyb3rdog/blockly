...
Blockly.TimePicker = function(isClock, is24H, showSecods, isLight, clockSize, clockFont, hour, minute, second) {
    ...
    this.clockSize = clockSize ?? 180;
    this.clockFont = clockFont ?? '16px Verdana';
...
Blockly.TimePicker.prototype.drawClock = function () {
    ...
    ctx.stroke();
    let radGrd = ctx.createRadialGradient(halfSize, halfSize, 1, halfSize, halfSize, halfSize);
...
Blockly.TimePicker.prototype.onPtrStart = function(x, y) {
    ...
    let last = this.isHourHand ? this.lastHourDeg : this.lastMinuteDeg
...
Blockly.TimePicker.prototype.getTimeString = function() {
    let result = ('0' + (this.is24H ? this.hour : this.hour % 12 == 0 ? 12 : this.hour % 12)).slice(-2) + ':' + ('0' + this.minute).slice(-2);
...
Blockly.TimePicker.prototype.setClockMode = function(m) {
    if (typeof m == 'boolean' && m != this.isClock) {
        ...
        //tpick.style.height=isClk?'240px':'240px';
        if (this.isClock) {
            this.unbindClockEvents();
            this.hour = Blockly.TimePicker.getHours();
            this.minute = Blockly.TimePicker.getMinutes();
